...
Node * insert_node(Node * current, int key) {
    ...
    if(key > current->key) current->right = insert_node(current->right, key);
    else if(key < current->key) current->left = insert_node(current->left, key);
...
void bst_push(BinarySearchTree * bst, int key) {
    ...
    bst->head = insert_node(bst->head, key);
}
